        ...
        main { class: "flex-1 min-h-0 mx-4 mb-0 mt-0 relative group/console",
            div { class: "absolute inset-0 bg-console-bg rounded-t-2xl border-t border-x border-[#222629] shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col",
                ...
                div {
                    ...
                    for (timestamp, text, base_class) in [
                        ...
                        ("[10:42:08]", "Warning: Pressure drift detected (-1hPa)", "text-gray-300"),
                    ...
                    ] {
                        {
                            ...
                            let mut is_visible = if filter_query.is_empty() {
                                ...
                                // Simple regex check (case insensitive by default unless we handle it)
                                if let Ok(re) = if match_case {
                                    ...
                                    true // If regex is invalid, don't filter out everything
                                }
                            } else {
                                if match_case {
                            ...
                            if is_visible {
                                ...
                                let current_highlights = highlights.clone();

                                for h in current_highlights.iter() {
                                                ...
                                                if part.starts_with("\x01") {
                                                    {
                                                        ...
                                                        let highlight_class = match color {
                                                        ...
                                                        rsx! { span { class: "{highlight_class}", "{match_text}" } }
                                                    }
                                                } else {
                                                    "{part}"
                                                }
